		...
		columnDefs: [
			...
			},
			{
				...
				render: function (data, type, row, meta) {
					return '<div class="btn-group" role="group">' + '<a data="' + data[0] + '" class="btn btn-warning btn-sm mdi mdi-tooltip-edit text-black btn_editar_consulta" data-toggle="tooltip" title="Editar">' + '<i class="fa fa-pen">Editar</i></a>' + '</div>';
				},
			},
		],
	});
	...
	$('#tbl_tratamiento_consultas').on('click', '.btn_editar_consulta', function (e) {
		...
		$.ajax({
			...
			.done(function (response) {
				...
				parametrosModal('#agregar-consulta', 'Editar consulta', 'modal-lg', false, true);
			})
			.fail(function (e) {
				$('#agregar-consulta').modal('hide');
			});
